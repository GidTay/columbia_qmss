...
This test_tictactoe.py script defines test functions
to test the Tic-Tac-Toe Game in tictactoe.py.

Functions for Question 1a to 1d and Question 2a:
- test_initialize_board()
- test_make_move_valid(player, row, col)
- test_make_move_invalid(player, row, col)
...
- test_valid_moves_fixture(set_board, player, row, col)
- test_out_of_bounds_moves_fixture(
    set_board, player, row, col, expected_result
    ):
- test_occupied_cell_fixture(set_board)
...
import pytest      # So we can run our tests with pytest
import tictactoe   # Import functions from tictactoe.py for testing


# Qn 1a: test_initialize_board()
...
def test_make_move_valid(player, row, col):
    ...
    valid_move = tictactoe.make_move(board, row, col, player)
...
def test_make_move_invalid(player, row, col):
    ...
    invalid_move = tictactoe.make_move(board, row, col, player)
...
def test_game_integration():
    ...
    for i, (player, row, col) in enumerate(moves):
        # Make a move
        tictactoe.make_move(board, row, col, player)
    ...
    assert tictactoe.check_winner(board) is None, "There should be no winner after resetting the game"
 

# Qn 2a: test_make_move()
@pytest.mark.parametrize(
    ...
    [
        ...
        *[
        ...
        # Test case 5: Out-of-bounds move (negative index), should raise IndexError
        (
            # Initial board
            [[' ', ' ', ' '],
             [' ', ' ', ' '],
            ...
            # Move with negative index should raise error
            -1, 0, 'X', pytest.raises(IndexError),
...
# Qn 2b: use pytest fixture functionality
...
def test_valid_moves_fixture(set_board, player, row, col):
    ...
    """
    valid_move = tictactoe.make_move(set_board, row, col, player)
    # Assert that the move was successful (return True)
    assert valid_move, (
        f"Move for player {player} at ({row}, {col}) "
        f"should be successful"
...
def test_out_of_bounds_moves_fixture(
    ...
    if isinstance(expected_result, type(pytest.raises(IndexError))):
        with expected_result:
            tictactoe.make_move(set_board, row, col, player)
...
def test_occupied_cell_fixture(set_board):
    ...
    result = tictactoe.make_move(set_board, 0, 0, 'O')
